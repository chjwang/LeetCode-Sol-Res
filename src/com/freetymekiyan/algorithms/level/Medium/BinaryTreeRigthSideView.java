...
public class BinaryTreeRigthSideView {
    ...
    public List<Integer> rightSideViewDFS(TreeNode root) {
        List<Integer> res = new ArrayList<>();
        dfsTraversal(res, root, 0);
        return res;
    }

    private void dfsTraversal(List<Integer> res, TreeNode node, int depth) {
        if (node == null) {
            return;
        }
        if (depth == res.size()) {
        ...
        dfsTraversal(res, node.right, depth + 1); // Recurse right node first
        dfsTraversal(res, node.left, depth + 1);
    }
}
